...
const text = value => value ? String(value).replace(/_/g, ' ') : '-';
...
export default function CheckOut() {
  ...
  if (stage === 'complete') return <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="card" style={{ maxWidth: 560, margin: '0 auto' }}><div className="card-b"><div className="success-box"><motion.div className="success-icon" initial={{ scale: 0 }} animate={{ scale: 1 }} style={{ background: 'var(--success-bg)', color: 'var(--success)' }}><UserMinus size={40} /></motion.div><h2>Visitor Checked Out</h2><p style={{ color: 'var(--text2)', marginBottom: 16 }}>Face verification was successful and the visit is now complete.</p><div className="detail-grid"><div className="detail-item"><div className="lbl">Visitor</div><div className="val">{nameOf(result?.visitor)}</div></div><div className="detail-item"><div className="lbl">Check-In</div><div className="val">{completedVisit?.checkInAt ? new Date(completedVisit.checkInAt).toLocaleTimeString() : '-'}</div></div><div className="detail-item"><div className="lbl">Check-Out</div><div className="val">{completedVisit?.checkOutAt ? new Date(completedVisit.checkOutAt).toLocaleTimeString() : '-'}</div></div><div className="detail-item"><div className="lbl">Purpose</div><div className="val">{text(completedVisit?.purpose)}</div></div></div><div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 20 }}><button className="btn-p" onClick={reset}>Back to Active Visitors</button><button className="btn-o" onClick={() => navigate('/history')}>View History</button></div></div></div></motion.div>;
  ...
  return <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
    <div className="card" style={{ marginBottom: 16 }}><div className="card-h"><h3><ArrowRightLeft size={16} style={{ marginRight: 8, verticalAlign: 'middle' }} />Checked-In Visitors</h3><span style={{ color: 'var(--text2)', fontSize: 13 }}>{visits.length} active</span></div><div className="card-b" style={{ padding: 0, overflowX: 'auto' }}><table className="tbl"><thead><tr><th>Visitor</th><th>Host</th><th>Purpose</th><th>Checked In</th><th style={{ textAlign: 'right' }}>Action</th></tr></thead><tbody>{visits.map(visit => <tr key={visit.id}><td><div className="vis-row"><div className="vis-av">{nameOf(visit.visitor).charAt(0)}</div><div className="vis-info"><h4>{nameOf(visit.visitor)}</h4><p>{visit.visitor?.visitorCode || ''}</p></div></div></td><td>{nameOf(visit.employee)}</td><td>{text(visit.purpose)}</td><td>{visit.checkInAt ? new Date(visit.checkInAt).toLocaleString() : '-'}</td><td style={{ textAlign: 'right' }}><button className="btn-p btn-sm" onClick={() => startVerification(visit)}>Verify & Checkout</button></td></tr>)}{!visits.length && <tr><td colSpan={5} style={{ textAlign: 'center', padding: 32, color: 'var(--text2)' }}>No visitors are currently checked in.</td></tr>}</tbody></table></div></div>

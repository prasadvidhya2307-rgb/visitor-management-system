...
export default function Profile() {
  ...
  const confirmSave = () => {
    setSaving(true);
    setTimeout(() => {
      let result = { ok: true, message: 'Profile updated successfully' };
      if (pw.next) {
        const res = store.changePassword(profile.email, pw.current, pw.next);
        if (!res.ok) { result = { ok: false, message: res.error }; }
      }
      if (result.ok) {
      ...
      setPwError(result.ok ? '' : result.message);
      if (result.ok) {
        setSaved(true);
